...
const passengerPost = async (req, res) => {
    ...
    try {
        ...
        const checkResult = await pool.query(checkQuery, [email_id, mobile_number]);
...
const passengerPut = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(passengerQuery, values);
    ...
    `;
    const referralResult = await pool.query(checkReferralQuery, [passengerId]);
    ...
    if (!referral_code && passengerName && passengerMobile && email_id && date_of_birth && gender && state && age && occupation) {
      ...
      await pool.query(updateReferralQuery, [newReferralCode, passengerId]);
    ...
    if (user_status && user_status.toLowerCase() === 'register') {
      ...
      const passengerEmailResult = await pool.query(passengerEmailQuery, [passengerId]);
...
const passengerDelete = async (req, res) => {
    ...
    try {
        ...
        const passengerResult = await pool.query(getPassengerQuery, [passengerId]);
        ...
        for (const query of deleteQueries) {
            await pool.query(query, [email, mobile_number]);
        }

        const passengerQuery = `DELETE FROM public.passenger_profile WHERE tbs_passenger_id = $1`;
        await pool.query(passengerQuery, [passengerId]);
...
const passengerGetById = async(req, res) =>{
    ...
    try {
        ...
        const result = await pool.query(passengerQuery, passengerValues)
...
const passengerGetAll = async(req, res) =>{
    try {
        ...
        const result = await pool.query(passengerQuery)
...
// GET-ById CONTROLLER OF WEB PASSENGERS AND ADD-PASSENGER DETAILS
   const AllpassengerSGetById = async (req, res) => {
    ...
    try {
      ...
      const result = await pool.query(query, [tbs_passenger_id]);
...
module.exports = { passengerPost, passengerPut, passengerDelete, passengerGetById, passengerGetAll, addpassengerDelete, addpassengerGetAll, addpassengerGetById, addpassengerPost, addpassengerPut, AllpassengerSGetById, passengerReferrelCodeGetById }