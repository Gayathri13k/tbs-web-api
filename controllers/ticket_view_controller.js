    ...
    try {
      ...
      const formattedResponse = result.rows.map(row => {
        ...
        return {
          ...row,
          DROPPING_POINT: droppingPoint.trim(),
          DROPPING_TIME: cleanDroppingTime,
          Dropping_Point_Time: undefined 
        };
      });
...
const CompletedJourney = async (req, res) => {
    ...
    try {
      const queries = `
        ...
        LEFT JOIN operators_logo od ON TRIM(LOWER(cj.operator_name)) = TRIM(LOWER(od.operator_name))
...
const CancelledJourney = async (req, res) => {
    ...
    try {
      const queries = `
        ...
        LEFT JOIN booking_details bd ON ct."Booking_Id" = bd."Booking_Id" 
        LEFT JOIN operators_logo od ON TRIM(LOWER(ct.operator_name)) = TRIM(LOWER(od.operator_name)) 
        WHERE ct.mobile_number = $1;`;
...
async function moveCompletedTickets() {
    ...
    try {
        ...
        await client.query('ROLLBACK');
        console.error('Error moving completed tickets:', error);
    } finally {
        client.release();
    }
}
